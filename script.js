    ...
    const songs = ['01 - Sgt. Pepper\'s Lonely Hearts Club Band.m4a', '02 - With a Little Help From My Friends.m4a', '03 - Lucy In the Sky With Diamonds.m4a', '04 - Getting Better.m4a', '05 - Fixing a Hole.m4a', '06 - She\'s Leaving Home.m4a', '07 - Being For the Benefit of Mr. Kite!.m4a', '08 - Within You Without You.m4a', '09 - When I\'m Sixty-Four.m4a', '10 - Lovely Rita.m4a', '11 - Good Morning Good Morning.m4a', '12 - Sgt. Pepper\'s Lonely Hearts Club Band (Reprise).m4a', '13 - A Day In the Life.m4a', '14 - Sgt. Pepper\'s Lonely Hearts Club Band (Take 9 and Speech).m4a', '15 - With a Little Help From My Friends (Take 1 , False Start and Take 2 , Instrumental).m4a', '16 - Lucy In the Sky With Diamonds (Take 1).m4a', '17 - Getting Better (Take 1 , Instrumental and Speech at the End).m4a', '18 - Fixing a Hole (Speech and Take 3).m4a', '19 - She\'s Leaving Home (Take 1 , Instrumental).m4a', '20 - Being For the Benefit of Mr. Kite! (Take 4).m4a', '21 - Within You Without You (Take 1 , Indian Instruments).m4a', '22 - When I\'m Sixty-Four (Take 2).m4a', '23 - Lovely Rita (Speech and Take 9).m4a', '24 - Good Morning Good Morning (Take 8).m4a', '25 - Sgt. Pepper\'s Lonely Hearts Club Band (Reprise) [Speech and Take 8].m4a', '26 - A Day In the Life (Take 1 With Hums).m4a', '27 - Strawberry Fields Forever (Take 7).m4a', '28 - Strawberry Fields Forever (Take 26).m4a', '29 - Strawberry Fields Forever (Stereo Mix 2015).m4a', '30 - Penny Lane (Take 6 , Instrumental).m4a', '31 - Penny Lane (Stereo Mix 2017).m4a'];
    const songNames = ['Sgt. Pepper\'s Lonely Hearts Club Band', 'With a Little Help From My Friends', 'Lucy In the Sky With Diamonds', 
							 'Getting Better', 'Fixing a Hole', 'She\'s Leaving Home', 'Being For the Benefit of Mr. Kite!', 'Within You Without You',
							 'When I\'m Sixty-Four', 'Lovely Rita', 'Good Morning Good Morning', 'Sgt. Pepper\'s Lonely Hearts Club Band (Reprise)',
							 'A Day In the Life', 'Sgt. Pepper\'s Lonely Hearts Club Band (Take 9 and Speech)', 'With a Little Help From My Friends (Take 1 , False Start and Take 2 , Instrumental)',
							 'Lucy In the Sky With Diamonds (Take 1)', 'Getting Better (Take 1 , Instrumental and Speech at the End)', 'Fixing a Hole (Speech and Take 3)', 
							 'She\'s Leaving Home (Take 1 , Instrumental)', 'Being For the Benefit of Mr. Kite! (Take 4)', 'Within You Without You (Take 1 , Indian Instruments)',
							 'When I\'m Sixty-Four (Take 2)', 'Lovely Rita (Speech and Take 9)', 'Good Morning Good Morning (Take 8)', 
							 'Sgt. Pepper\'s Lonely Hearts Club Band (Reprise) [Speech and Take 8]', 'A Day In the Life (Take 1 With Hums)',
							 'Strawberry Fields Forever (Take 7)', 'Strawberry Fields Forever (Take 26)', 
							 'Strawberry Fields Forever (Stereo Mix 2015)', 'Penny Lane (Take 6 , Instrumental)', 'Penny Lane (Stereo Mix 2017)'];
	...
	function loadSong() {
		...
		albumCover.src = `media/Sgt. Pepper's Lonely Hearts Club Band - CD.png`;

		// 如果找不到专辑封面，则加载 default.png
		albumCover.onerror = function() {
			albumCover.src = 'html/default.png';
		};
   ...
   // loadSong()
      ...
      albumCover.src = 'html/default.png'; // 停止音乐后回到默认专辑封面
    }
    ...
    function playPauseMusic() { // 播放/停止按钮:
      ...
      if (isPlaying==true) {
	    ...
	    albumCover.src = `media/Sgt. Pepper's Lonely Hearts Club Band - CD.png`; // 恢复音乐后恢复当前曲目专辑封面	
      }
    }
...
*/
	...
	function toggleMenu() { // 菜单:
	  // 获取菜单元素
	  const menu = document.getElementById('menu');
	  // 切换菜单的 "open" 类（展开或收起菜单）
	  menu.classList.toggle('open');

	  // 获取所有带有 "line" 类的元素（通常是汉堡菜单的三条线）
	  const lines = document.querySelectorAll('.line');
	  // 获取菜单按钮（即汉堡菜单）的高度
	  const menuHeight = document.querySelector('.hamburger-menu').offsetHeight;
	  // 计算移动距离，用于平移动画
	  const translateValue = menuHeight / 1.55;

	  // 遍历每一条线，根据菜单是否打开来设置动画
	  lines.forEach((line, index) => {
		if (menu.classList.contains('open')) {
		  // 如果菜单是打开状态
		  if (index === 0) {
			// 第一条线旋转 45 度并向下平移
			line.style.transform = `rotate(45deg) translateY(${translateValue}px)`;
		  } else if (index === 1) {
			// 第二条线隐藏（透明度为 0）
			line.style.opacity = '0';
		  } else if (index === 2) {
...
/*
	...
	document.addEventListener("DOMContentLoaded", function () {
	  if (window.innerWidth <= 980) {
		...
		if (video) {
		  video.parentElement.removeChild(video); // 或者 video.remove();
		}
	  }
	})
*/
	...
	video.addEventListener('play', () => {
		if (!video.paused) {
			...
			reminderMessage.id = 'reminder-message';
			reminderMessage.innerText = 'Click the screen to toggle mute state'; // 提示文字
			document.body.appendChild(reminderMessage);

			// 设置定时器，几秒后隐藏提醒消息
			setTimeout(() => {
				reminderMessage.style.transition = 'opacity 1s'; // 淡出效果
				reminderMessage.style.opacity = '0';
				setTimeout(() => {
					reminderMessage.remove(); // 移除提醒消息
				}, 1000); // 等待淡出完成后移除
			}, 5000); // 提示消息显示 5 秒
			...
			setTimeout(() => {
				...
				skipText.innerText = 'Skip'; // 显示“Skip”文字
				document.body.appendChild(skipText);

				// 点击“Skip”文字时结束视频播放
				skipText.addEventListener('click', () => {
					video.pause(); // 暂停视频
					video.currentTime = video.duration; // 将视频进度设置为结束状态
					videoContainer.style.transition = 'opacity 1s'; // 淡出效果
					videoContainer.style.opacity = '0';
					setTimeout(() => {
						videoContainer.style.display = 'none'; // 隐藏视频容器
					}, 1000); // 确保淡出完成后隐藏
					skipText.remove(); // 视频结束后删除跳过按钮
				});
			}, 6500); // 延迟 5 秒创建“Skip”文字
		}
	});
	...
	video.addEventListener('ended', () => {
		skipText.remove(); // 视频结束后删除跳过按钮
		video.style.transition = 'opacity 1s'; // 设置淡出过渡效果
		video.style.opacity = '0'; // 开始淡出
		setTimeout(() => {
			video.style.display = 'none'; // 确保完全淡出后隐藏
		}, 1000); // 设置淡出的时间
		// 动态更改video-background-container:z-index
		videoContainer.style.zIndex = '-1000';
		console.log('#video-background-container z-index 已更改为 -1000');
	});

	// 点击网页 body 时切换静音状态
	body.addEventListener('click', (event) => {
		if (!video.muted && video.paused)
			return; // 如果视频未静音且不在播放状态，不执行操作
		else (!video.paused)
			video.muted = !video.muted; // 取消静音/开启静音
	});
        ...
        window.onload = function () {
            function ClickFrontShow() {
                this.fron = ['🎵', '🎶', '🎼',
							 ...
							 'Strawberry Fields Forever (Take 7)', 'Strawberry Fields Forever (Take 26)', 
							 'Strawberry Fields Forever (Stereo Mix 2015)', 'Penny Lane (Take 6 , Instrumental)', 'Penny Lane (Stereo Mix 2017)',
							 '1967', 'John Lennon', 'Paul McCartney', 'George Harrison', 'Ringo Starr', 'The Beatles'];
